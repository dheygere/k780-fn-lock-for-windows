...
This program sets the function keys for the Logitech K780 bluetooth keyboard on Windows. 
...
{
    int seq_len = 7;
    const unsigned char k780_seq_fkeys_on[] = {0x10, 0x02, 0x0c, 0x1c, 0x00, 0x00, 0x00};
    const unsigned char k780_seq_fkeys_off[] = {0x10, 0x02, 0x0c, 0x1c, 0x01, 0x00, 0x00};
    int k780_vid = 0x46d;
    int k780_pid = 0xc52b;
    ...
    devs = hid_enumerate(k780_vid, k780_pid);
    cur_dev = devs;
    ...
    while (cur_dev)
    {
        if (cur_dev->usage == TARGET_USAGE && cur_dev->usage_page == TARGET_USAGE_PAGE)
        {
            ...
            res = hid_write(handle, k780_seq_fkeys_off, seq_len);
#else
            printf("Set function keys as default");
            res = hid_write(handle, k780_seq_fkeys_on, seq_len);
#endif
